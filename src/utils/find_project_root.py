...
def find_project_root(
    ...
    if markers is None:
        ...
        markers = [
            ...
            # e.g., 'package.json', '.env'
        ]

    if not isinstance(start_path, str | pathlib.Path):
        raise TypeError(
            f"start_path must be a string or Path object, not {type(start_path)}"
        )
    ...
    try:
        ...
        raise FileNotFoundError(
            f"The starting path '{start_path}' does not exist or could not be resolved."
        ) from err
