...
def make_figure_1(fm, data, VERBOSE):
    ...
    axes[0] = generate_plot(data, x='school', y='wage', plot_type='plot', x_bins=5, group_by='residence', agg_fct=pl.mean, ax = axes[0], verbose=VERBOSE, xlabel='School', ylabel='Wage', title='Average Wage by School and Residence')
    axes[1] = generate_plot(data, x='exper', y='wage', plot_type='plot', x_bins=5, group_by='residence', agg_fct=pl.mean, ax = axes[1], verbose=VERBOSE, xlabel='Experience', ylabel='Wage', title='Average Wage by Experience and Residence')
    axes[2] = generate_plot(data, x='wage', bins=30, plot_type='hist', group_by='maried', ax = axes[2], verbose=VERBOSE, plot_settings={'alpha': 0.5})
...
def make_figure_2(fm, data):
    ...
    axes[0] = generate_plot(cd, x="school", y="wage_mean", y_err="wage_std", ax = axes[0], xlabel='School years', ylabel='Wage', title='Average Wage by School and Residence', label='Wage')
    axes[0] = generate_plot(cd, x="school", y="exper_mean", y_err="exper_std", ax = axes[0], xlabel='School years', ylabel='Experience', title='Average Experience by School and Residence', label='Experience')
    axes[1] = generate_plot(cd, x="school", y="wage_mean", y_err=("wage_ci_low", "wage_ci_high"), ax = axes[1], xlabel='School years', ylabel='Wage', title='Average Wage by School and Residence', label="Wage")
    axes[1] = generate_plot(cd, x="school", y="exper_mean", y_err=("exper_ci_low", "exper_ci_high"), ax = axes[1], xlabel='School years', ylabel='Experience', title='Average Experience by School and Residence', label="Experience")
